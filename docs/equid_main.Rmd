...
address:
  ...
  - code: Universidade de Lisboa
    address: Centro de Arqueologia da Universidade de Lisboa, Faculdade de Letras da Universidade de Lisboa, Alameda da Universidade, Lisboa, Portugal (1600-214)
...
(ref:equidmarkerplot-label) MALDI spectra of chymotryptic peptides of COL1 for horse (blue) and donkey (brown). The majority of the peaks present in the spectra are identical (upper), with the major difference between the two spectra being the diagnostic marker with horse at m/z 2497 and donkey at m/z 2511 (lower). The sequence of the peptide confirmed by LC-MS/MS is shown with the single amino acid difference between the two species highlighted in red.

```{r equidmarkerplot, dpi=600, echo=FALSE, dpi=600, fig.show="hold", out.width='0.98\\textwidth',fig.align='center',fig.height=8,crop = NULL, fig.env='figure*', fig.cap= '(ref:equidmarkerplot-label)'}
...
horse_donkey_zoom<- ggplot(NULL) + geom_line(data = df_ref_horse, aes(x=mass,y=intensity_norm),colour='#4682B4',size=0.5) + geom_line(data = df_ref_donkey_flip, aes(x=mass,y=intensity2),colour='#B47846',size=0.5) + theme + labs(x='m/z', y='Relative Intensity') + scale_x_continuous(limits=c(2450,2550),breaks=seq(2450,2550,10)) + scale_y_continuous(limits=c(-30,50),breaks=seq(-30,50,5)) + annotate(geom = "point", x = 2497.2, y = 26, colour = "#B4464B", size = 1) + annotate(geom = "text", x = 2496.8, y = 31, colour = "black", size = 2, label = '2497',angle=90) + annotate(geom = "point", x = 2511.2, y = -15, colour = "#B4464B", size = 1) + annotate(geom = "text", x = 2511, y = -20, colour = "black", size = 2, label = '2511',angle=90) + geom_hline(yintercept = 0, color='black',size=0.5) + annotate("text", x = 2450, y=45, hjust = 0, parse=T, label='"GRTGDAGPVGPPGPPGPPGPPGPPS" * phantom("GGFDFSFLP")', color="black", size = 2) + annotate("text", x = 2450, y=45, hjust = 0, parse=T, label='phantom("GRTGDAGPVGPPGPPGPPGPPGPPS") * "G" * phantom("GFDFSFLP")', color="red", size = 2) + annotate("text", x = 2450, y=45, hjust = 0, parse=T, label='phantom("GRTGDAGPVGPPGPPGPPGPPGPPSG") * "GFDFSFLP"', color="black", size = 2) + annotate("text", x = 2450, y=-30, hjust = 0, parse=T, label='"GRTGDAGPVGPPGPPGPPGPPGPPS" * phantom("AGFDFSFLP")', color="black", size = 2) + annotate("text", x = 2450, y=-30, hjust = 0, parse=T, label='phantom("GRTGDAGPVGPPGPPGPPGPPGPPS") * "A" * phantom("GFDFSFLP")', color="red", size = 2) + annotate("text", x = 2450, y=-30, hjust = 0, parse=T, label='phantom("GRTGDAGPVGPPGPPGPPGPPGPPSA") * "GFDFSFLP"', color="black", size = 2)


horse_donkey_plot <- ggplot(NULL) + geom_line(data = df_ref_horse, aes(x=mass,y=intensity_norm),colour='#4682B4',size=0.5) + geom_line(data = df_ref_donkey_flip, aes(x=mass,y=intensity2),colour='#B47846',size=0.5) + theme + labs(x='m/z', y='Relative Intensity') + scale_x_continuous(limits=c(1000,3200),breaks=seq(1000,3200,200)) + annotate("text",x = 3000,y = 80,colour = "black" ,size = 3,label = expression(italic("Equus caballus"))) + annotate("text",x = 3000,y = -80,colour = "black" ,size = 3,label = expression(italic("Equus asinus"))) + geom_hline(yintercept = -0.5, color='black',size=0.5) + annotate("rect", xmin = 2450, xmax = 2550, ymin = -Inf, ymax = Inf,alpha = .2,colour='gray')



horse_donkey_plot/horse_donkey_zoom
```
...
colo_eq <- c("Equus" = "#808080", "Equus caballus" = "#4682B4", "Equus asinus" = "#B47846")
